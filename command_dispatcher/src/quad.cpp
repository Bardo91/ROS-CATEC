 ...
 *		Modification:	Pablo R.S.	2015-FEB-17
 */
...
class_quad::class_quad(int _id, double _x, double _y, double _z, double _vMax, double _range, double _tpo):
  			id(_id), x(_x), y(_y), z(_z), vel_max(_vMax), range(_range), tpo_min_cont(_tpo), estado(1), id_contactado(-1), tarea_nueva(1), coste_mision(0.0) {
...
struct Costes class_quad::calcula_coste (int plan [MAX_TASKS], double tasks_cont [MAX_TASKS][TAM_TASKS], struct Agente a) {
	...
	if (plan[0]==-1){
		...
		int pos_dest[2]={int(tasks[plan[0]][3]),int(tasks[plan[0]][4])};
