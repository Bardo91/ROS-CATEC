...
void tOff_Done_CB(const actionlib::SimpleClientGoalState& state,
                  const TakeOffResultConstPtr& result);
void land_Active_CB();
void land_Feedback_CB(const LandFeedbackConstPtr& feedback);
void land_Done_CB(const actionlib::SimpleClientGoalState& state,
                  const LandResultConstPtr& result);
...
int main(int argc, char **argv)
{
   ...
   ros::Publisher joystick_publisher =
         n.advertise<ControlReferenceRwStamped> (topicname.c_str(), 0);
   ...
   LandClient landActionClient(topicname);
   //Create takeoff and land clients
   topicname = "/uav_";
   topicname.append(argv[1]);
   topicname.append("/take_off_action");
   TakeOffClient takeoffActionclient(topicname);

   //Wait for takeoff and land servers
   takeoffActionclient.waitForServer(ros::Duration(0));
   landActionClient.waitForServer(ros::Duration(0));
   ...
   while(ros::ok())
   {
      ...
      {
         tf::Vector3 quad_desired_position,
               quad_desired_position_unity, quad_calculated_position;
         ...
         length_scaled = length / (1024 / max_cruise);
         //1024 is maximum input by PS3 joystick; therefore we limit length_scaled to 1.5m (only using one stick!)
         ...
         {
            //Pitch
            control_ref_to_send.c_reference_rw.position.x =
                  last_ual_state.ual_state.dynamic_state.position.x + quad_calculated_position.x();


            //Roll
            control_ref_to_send.c_reference_rw.position.y =
                  last_ual_state.ual_state.dynamic_state.position.y + quad_calculated_position.y();


            //Thrust
            control_ref_to_send.c_reference_rw.position.z =
                  last_ual_state.ual_state.dynamic_state.position.z + quad_calculated_position.z();
         ...
         }else if(positionSentLastTime)
         {
            ...
            control_ref_to_send.c_reference_rw.position.x =
                  last_ual_state.ual_state.dynamic_state.position.x;
            control_ref_to_send.c_reference_rw.position.y =
                  last_ual_state.ual_state.dynamic_state.position.y;
            control_ref_to_send.c_reference_rw.position.z =
                  last_ual_state.ual_state.dynamic_state.position.z;
         ...
         {
            control_ref_to_send.c_reference_rw.heading =
                  last_ual_state.ual_state.dynamic_state.orientation.z + (wjse.stick2_y/3000.0);
         }
      }
      ...
      if((joystick.get_joystick_status(&wjse)!=-1) && (wjse.button[0] == 1))
      {
         ...
         takeoffActionclient.sendGoal(tOff_goal, &tOff_Done_CB, &tOff_Active_CB, &tOff_Feedback_CB);
      ...
      if((joystick.get_joystick_status(&wjse)!=-1) && (wjse.button[1] == 1))
      {
         ...
         LandGoal land_goal;
         landActionClient.sendGoal(land_goal, &land_Done_CB, &land_Active_CB, &land_Feedback_CB);
         usleep(500000); // In order to not send it twice accidentally
      }

//      std::cout << "joystick_publisher: "
//                << control_ref_to_send.c_reference_rw.position.x << std::endl;
