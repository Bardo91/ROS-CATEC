...
Teleop::Teleop(string nquad) :
	name(nquad), vx(0), vy(0), h(1), heading(0), x(0), y(0), counter(0) {
...
int main(int argc, char** argv) {
	...
	ros::init(argc, argv, "posjoystick");
...
void Teleop::keyLoop() {
	...
	while (!done) {
		double at = (ros::Time::now()-t).toSec();
		t = ros::Time::now();
		...
		if (rc == 1) {
			...
			case 0:
				if (jse.type == 2) {
					vy = -1 * ((double) jse.value) / 32767.0f;
					dirty = true;
			...
			case 1:
				if (jse.type == 2) {
					vx = -1 * ((double) jse.value) / 32767.0f;
					dirty = true;
			...
			case 3:
				if (jse.type == 2) {
					h = abs(-2 * ((double) jse.value - 32767) / 65534.0f);
					dirty = true;
					cout << "h: " << h << endl;
				}
				break;
