...
void tOff_Done_CB(const actionlib::SimpleClientGoalState& state, const TakeOffResultConstPtr& result)
{
	...
	if(state.state_==state.SUCCEEDED || result->result==TakeOffResult::ALREADY_FLYING)
	{
		...
		WayPointWithCruiseStamped wayPoint0;
		wayPoint0.way_point.cruise = 1.5;
		wayPoint0.way_point.x = 0;
		wayPoint0.way_point.y = 0;
		wayPoint0.way_point.z = 0;

		WayPointWithCruiseStamped wayPoint1;
		wayPoint1.way_point.cruise = 1;
		wayPoint1.way_point.x = -5;
		wayPoint1.way_point.y = 5;
		wayPoint1.way_point.z = 2;

		WayPointWithCruiseStamped wayPoint2;
		wayPoint2.way_point.cruise = 1.5;
		wayPoint2.way_point.x = 5;
		wayPoint2.way_point.y = -5;
		wayPoint2.way_point.z = 0;

		WayPointWithCruiseStamped wayPoint3;
		wayPoint3.way_point.cruise = 2;
		wayPoint3.way_point.x = 5;
		wayPoint3.way_point.y = 5;
		wayPoint3.way_point.z = 2;
		goal.way_points.push_back(wayPoint0);
		goal.way_points.push_back(wayPoint1);
		goal.way_points.push_back(wayPoint2);
		goal.way_points.push_back(wayPoint3);
		goal.size = goal.way_points.size();

		cWayPoint->sendGoal(goal,&gTWP_Done_CB,&gTWP_Active_CB,&gTWP_Feedback_CB);
		gTWPGoalSended = true;
	}
