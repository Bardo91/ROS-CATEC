...
//My next waypoint
WayPointWithCruiseStamped my_next_waypoint[3];

//nextWayPoint
ros::Publisher my_waypoint_pub[3];

//Last state of our uav
UALStateStamped last_ual_state[3];
...
LandClient *cLand2;
TakeOffClient *cTakeOff2;

bool LandGoalSended[3];
bool TakeOffGoalSended[3];
bool volando[3];
int num_ag;
int num_intruders;
int cambio[3];
...
double pos_inicial[3][2];
TakeOffGoal tOff_goal[3];

ofstream pos_quads[3];
ofstream pos_intruders[3];
ofstream task_assigned[3];
ofstream costes_totales[3];
...
void land_Active_CB2();
void land_Feedback_CB2(const LandFeedbackConstPtr& feedback);
void land_Done_CB2(const actionlib::SimpleClientGoalState& state, const LandResultConstPtr& result);

void tOff_Active_CB2();
void tOff_Feedback_CB2(const TakeOffFeedbackConstPtr& feedback);
void tOff_Done_CB2(const actionlib::SimpleClientGoalState& state, const TakeOffResultConstPtr& result);
/*
 * Callbacks for Land
 *
 *
...
//volando[1]=true;
	...
	pos_inicial[1][1]=last_ual_state[1].ual_state.dynamic_state.position.y;
	sleep(10);
	cTakeOff2->sendGoal(tOff_goal[2], &tOff_Done_CB2, &tOff_Active_CB2, &tOff_Feedback_CB2);
}

void tOff_Active_CB2()
{
	//Se ejecutara cuando la tarea pase al estado activo
	pos_inicial[2][0]=last_ual_state[2].ual_state.dynamic_state.position.x;
	pos_inicial[2][1]=last_ual_state[2].ual_state.dynamic_state.position.y;
	ROS_INFO("Take Off Active.");
}
...
void UAV_StateCallBack2(const UALStateStamped::ConstPtr& state) 
{
	last_ual_state[2] = *state;
}
...
int main(int argc, char** argv)
{
	...
	for (int i=0; i<MAX_TASKS; i++)
		...
		topicname=uav_full_id[2];
		topicname.append("/ual_state");
		sub[2] = n.subscribe(topicname.c_str(), 0,UAV_StateCallBack2);
		...
		topicname = uav_full_id[2];
 		topicname.append("/land_action");
		cLand2 = new LandClient(topicname,true);
		topicname = uav_full_id[2];
		topicname.append("/take_off_action");
		cTakeOff2 = new TakeOffClient(topicname,true);  		
	...
	cLand2->waitForServer();
	cTakeOff2->waitForServer();
