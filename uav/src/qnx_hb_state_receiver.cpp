...
ros::Publisher testbed_state_pub[10];
ros::Publisher testbed_altimeter_pub[10];
...
int main(int argc, char** argv)
{
		...
		for(int i=1;i<=10;i++)
		{
			...
			testbed_state_pub[i-1] = n.advertise<UALStateStamped> (topicname.c_str(), 0);
			testbed_altimeter_pub[i-1] = n.advertise<std_msgs::Float64> (sensorTopicname.c_str(), 0);
		}
		...
		while(ros::ok())
		{
			if(!udp_server.receive(&received_state,sizeof(IUavState),(sockaddr*)&si_other,(socklen_t*)&slen))
			{
				cerr << "Testbed_State: Error reading udp datagram..." << endl;
			}
			else if(received_state.uavId.uiUavId <= 10 && received_state.uavId.uiUavId > 0)
			{
				//cerr << "Received data size: "<< sizeof(IUavState) << endl;
				//cerr << "Received state from Interface: "<< received_state.posRel.dXRel <<" UavID: "<< received_state.posRel.dYRel  << endl;
...
//TODO: Warning the order was worse. This change could affect to other modules
			...
			else
			{
				cerr << "Testbed_State: state received from unknow uav... Interface: "
					 << received_state.state.uiIdInterface <<" UavID: "<< received_state.uavId.uiUavId  << endl;
			}
