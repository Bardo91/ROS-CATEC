...
#define PORT 	   5225
#define QNX_SERVER "10.0.0.40" //Qnx Tierra  //"10.0.0.62" (Penguin)
...
void testbedStateCallback(const catec_msgs::UALStateStamped::ConstPtr& fp)
{
   ...
   {
	   batt_= battery_model_->update(
				   last_scientific_info_.enable, actual_state!=LANDED);
   ...
   state.ual_state.remaining_battery = (batt_/(30.0*60.0))*100;
